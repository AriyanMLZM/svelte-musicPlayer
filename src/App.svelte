<script lang="ts">
	...
	let index: number = 0
	let selectedSong: IMusic

	$: selectSong(index)

	const selectSong = (ind: number) => {
		index = ind
		selectedSong = musics[ind]
...
<main class="w-full h-screen md:flex-row-reverse md:flex">
	<section
		style="background-image: url({selectedSong.cover});"
	...
	>
		<div class="w-full h-full flex-center bg-black/60 backdrop-blur-[15px]">
			...
			>
				...
				<Player {selectedSong} {songOperation} />
			</div>
		</div>
	</section>
	<section class="md:w-[30%] w-full h-full">
		<MusicList {selectSong} activeIndex={index} />
	</section>
</main>
